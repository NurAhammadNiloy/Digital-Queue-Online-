-- No new tables. One session-bound transaction for metadata, credentials and
-- permission changes. Existing FK/RLS rules and session revocation triggers stay.
create function public.manage_staff(
...
begin
  ...
  if p_operation in ('create', 'update') then
    if p_name is null or length(btrim(p_name)) not between 1 and 200 or p_name !~ '[^[:space:]]'
      or p_staff_code is null or upper(btrim(p_staff_code)) !~ '^[A-Z0-9][A-Z0-9_-]{2,31}$'
