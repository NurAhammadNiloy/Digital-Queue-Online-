...
export default async function LocationPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ services?: string }> }) {
  ...
  const posterUrl = `/api/manager/locations/${id}/qr?poster=1`;
  return <main className="page">
    <Breadcrumbs section="Locations" href="/manager/locations" current={location.name} />
    <section className="card space-y-4" aria-label="Location summary">
    ...
    {!location.archived_at && <section className="card break-inside-avoid" aria-label="Location QR code">
      <div className="flex flex-col items-start gap-5 sm:flex-row sm:items-center">
        ...
        <div className="min-w-0 flex-1 space-y-3"><h2>Public queue</h2><p className="break-all text-sm text-slate-600">{url}</p>
          ...
          <div className="flex flex-wrap gap-2"><a className="btn" href={url} target="_blank" rel="noreferrer">Open queue page<span className="sr-only"> (new tab)</span></a><CopyButton value={url} /><a className="btn" href={`${posterUrl}&download=1`} download>Download poster</a><PrintButton href={posterUrl} /></div>
        </div>
      </div>
    </section>}
    {location.archived_at && <Link className="btn" href={`/manager/analytics?locationId=${id}`}>View historical analytics</Link>}
    <section className="space-y-4" aria-label="Services management"><div><h2>Services</h2><p className="muted mt-1">Manage queue settings and availability for this location.</p></div>
